...
如果对方有不只一个公钥，但你只认证过其中的一部分，或你见到曾经认证过公钥的对方在握手时传来了一个从未见过的新公钥，那么你可以让对方用已被你认证过的公钥（所在的实例）和你握手后把他使用的公钥的指纹发给你（其效力和他当面把指纹交给你无异，只要对方没有把你认证过的公钥对应的私钥交给他人），然后你便可以在 OTR 插件的配置对话框中打开公钥指纹列表手动核对，并标记通过核对的公钥。
...
### 关于问答共享密钥的身份认证
pidgin的otr插件默认支持问答共享密钥方式的身份认证，多通道是这种认证模式的核心，这里介绍几种常见的方式：

* 在咖啡厅，酒吧或者club里见面时交换密钥，物理上见面是相对安全的方式，但也需要把窃听者纳入威胁建模，在物理地点进行otr共享密钥的交换在实际场景中通常是在参加PGP signing party时顺带完成。

* 使用PGP邮件交换信息，此种方式有两个地方需要注意：1) 验证对方的KEY是否收到其他第三方信任 2) 核对fingerprint(通常也是物理见面) 使用此种方法的前提是要保证PGP本身的安全性

* 在安全性需求较低的情况，可以使用电话，短信和其他即时聊天工具完成。安全性较高的场景则可以使用暗网，比如基于洋葱网络或者需要SSL/TLS/SSH认证的内网聊天服务。

* 另外，在以上集中模式的基础上也可以继续拆分多通道信息交换，比如通过多个通道获得的信息的组合才是最终的共享密钥。
